...
plt.rcParams['font.family'] = 'Segoe UI Emoji'  # Use Segoe UI Emoji for Windows
...
def generate_map_image(village, output_path):
    ...
    gdf_roads = gdf[gdf['highway'].notna()]  # Assumes 'highway' tag exists
    gdf_buildings = gdf[gdf['building'].notna()]  # Assumes 'building' tag exists
    gdf_parks = gdf[gdf['landuse'] == 'park']  # Assumes 'landuse=park' for parks
    gdf_gardens = gdf[gdf['amenity'] == 'garden']  # Assumes 'amenity=garden' for gardens
    gdf_water = gdf[gdf['natural'] == 'water']  # Assumes 'natural=water' for water bodies
    gdf_landmarks = gdf[gdf['amenity'].isin(['school', 'hospital', 'place_of_worship'])]  # Schools, hospitals, temples
    ...
    if village.infrastructure['lakes'] > 0:
        landmarks.append(('Lake', '🏞️', Point(village.location.x - 0.01, village.location.y + 0.01)))  # Lake
    if village.infrastructure['temples'] > 0:
        landmarks.append(('Temple', '🛕', Point(village.location.x, village.location.y)))  # Temple
    if village.number_of_schools > 0:
        landmarks.append(('School', '🏫', Point(village.location.x + 0.01, village.location.y - 0.01)))  # School
    ...
    ax.set_facecolor('aliceblue')  # Light blue background for a map-like feel
